...
let objectIntFieldText = (raw, fieldName) =>
  switch raw->toUnknown->Surrealdb_Value.fromUnknown {
  | Object(entries) => entries->Dict.get(fieldName)->Option.map(Surrealdb_Value.toText)
  | None => None
  | _ => Some("<unexpected>")
  }
